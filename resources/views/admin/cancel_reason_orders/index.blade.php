...
@section('title')
    {{__('admin.paneltitle')}} | {{__('admin.faqtitle')}}
@endsection
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            @component('admin.components.search')
                @slot('url')
                    {{url('admin/cancel-reason-orders/search')}}
                @endslot
            @endcomponent
        </div>
            ...
            @component('admin.components.panel')
                @slot('header')
                    <h3 class="card-title" style="display: inline">
                        <i class="fa fa-clipboard   mr-2"></i>{{__('admin.cancel_reason_orders_tableheader')}}
                    </h3>
                    <a href="{{url('/admin/cancel-reason-orders/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> {{__('admin.cancel_reason_orders_tableheader')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $cancel_reason_orders , 'selects' => ['id' , 'description'], 'options' => ['edit' , 'delete']])
                        @slot('paginate')
                            {{$cancel_reason_orders->links()}}
                        @endslot
                        @slot('url')
                            cancel-reason-orders
                        @endslot
...
@section('scriptCustom')
    ...
    @component('admin.components.script.paginatorScript' , ['type' => 2])
        @slot('paginatorUrl')
            cancel-reason-orders/load?faq=
        @endslot
    @endcomponent
    @component('admin.components.script.searchScript')
        @slot('url')
            ../../../admin/cancel-reason-orders/search/
        @endslot
    @endcomponent
    @component('admin.components.script.sortScript')
        @slot('url')
            ../../../admin/cancel-reason-orders/sort/
...
@section('crumb')
    @component('admin.components.crumb')
        ...
        @slot('items')
            ...
            <li class="breadcrumb-item active"><i class="fe fe-paperclip mr-2 fs-14"></i>{{__('admin.faqtitle')}}
            </li>
        @endslot
    @endcomponent
@endsection
