...
@section('main')
    <form class="form" method="post" id="mainForm" enctype="multipart/form-data"
          action="{{url('admin/admin-invoices/update/' . $invoice->id)}}">
        @csrf
        <div class="row">
                ...
                @component('admin.components.panel')
                    @slot('items')
                        ...
                        @component('admin.components.form.optionLabel')
                                ...
                                @slot('items')
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{$user->id}}" @if($user->id == $invoice->user_id) selected @endif>{{$user->name}}</option>
                                    @endforeach
                                @endslot
                        @endcomponent
                        @component('admin.components.form.optionLabel')
                            ...
                            @slot('items')
                                ...
                                @foreach($countries as $country)
                                    <option value="{{$country->id}}" @if($country->id == $invoice->country_id) selected @endif>{{$country->name}}</option>
                                @endforeach
                            @endslot
                        @endcomponent
                        ...
                        <div class="form-group row">
                            ...
                            <textarea class="form-control" name="description" id="description" placeholder="" cols="30" rows="10">{{$invoice->description}}
                            </textarea>
                            </div>
                        </div>
                    @endslot
                ...
                @component('admin.components.panel')
                    ...
                    @slot('items')
                        @component('admin.components.form.submit')
                            ...
                            @endslot
                        @endcomponent
                        <a href="{{url('admin/admin-invoices')}}" class="btn btn-danger btn-block ">{{__('admin.cancel')}}</a>
                    @endslot
                @endcomponent
