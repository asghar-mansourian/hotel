        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/calculatores/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> {{__('admin.addregion')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $calculatores , 'selects' => ['id' , 'from' , 'to' , [ 'country','name'] , 'currency','liquid_currency'] , 'options' => ['edit' , 'delete']])
                        @slot('paginate')
                            {{$calculatores->links()}}
                        @endslot
                        @slot('url')
