        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/customers/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> Add
                        {{__('admin.customers')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $customers , 'selects' => ['id' , 'name','link','customer_image'], 'options' => ['edit' , 'delete']])
                        @slot('paginate')
                            {{$customers->links()}}
                        @endslot
                        @slot('url')
