...
@section('title')
    {{__('custom.admin.panel.title')}} | {{__('admin.adminPanelHowWorkTitle')}}
@endsection
        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/how_we_work/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i>
                        {{__('admin.addhowWork')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $howWeWorks , 'selects' => ['id' , 'title','title_ru','description','description_ru'], 'options' => ['edit' , 'delete']])
                        @slot('paginate')
                            {{$howWeWorks->links()}}
                        @endslot
                        @slot('url')
