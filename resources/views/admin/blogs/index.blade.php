        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/blogs/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i>{{__('admin.addblog')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $blogs , 'selects' => ['id' , 'title' ], 'options' => ['show' , 'edit' , 'delete']])
                        @slot('paginate')
                            {{$blogs->links()}}
                        @endslot
                        @slot('url')
...
@section('crumb')
    @component('admin.components.crumb')
        ...
        @slot('items')
            ...
            <li class="breadcrumb-item active"><i class="fe fe-paperclip mr-2 fs-14"></i>{{__('admin.blogtitle')}}
            </li>
        @endslot
    @endcomponent
@endsection
