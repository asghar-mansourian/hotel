...
@section('title')
    {{__('admin.dashboard')}} | {{__('admin.permission_title')}}
@endsection
            ...
            @component('admin.components.panel')
                @slot('header')
                    <h3 class="card-title" style="display: inline">
                        <i class="fa fa-clipboard   mr-2"></i>Manage {{__('admin.permission_title')}}
                    </h3>
                    <a href="{{url('/admin/permissions/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> Add Permission </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $permissions , 'selects' => ['id' , 'name'], 'options' => ['edit' , 'delete']])
                        @slot('paginate')
                            {{$permissions->links()}}
                        @endslot
                        @slot('url')
...
@section('crumb')
    @component('admin.components.crumb')
        ...
        @slot('items')
            ...
            <li class="breadcrumb-item active"><i class="fe fe-paperclip mr-2 fs-14"></i>{{__('admin.permission_title')}}
            </li>
        @endslot
    @endcomponent
@endsection
