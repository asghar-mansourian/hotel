...
@section('title')
    {{__('admin.dashboard')}} | {{__('admin.admins')}}
@endsection
        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/admins/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> Add Admin </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $admins , 'selects' => ['id' , 'name', 'family', 'email' , 'status' ], 'options' => ['edit' , 'delete','assign_role']])
                        @slot('paginate')
                            {{$admins->links()}}
                        @endslot
                        @slot('url')
...
@section('crumb')
    @component('admin.components.crumb')
        ...
        @slot('items')
            ...
            <li class="breadcrumb-item active"><i class="fe fe-paperclip mr-2 fs-14"></i>{{__('admin.admins')}}
            </li>
        @endslot
    @endcomponent
@endsection
