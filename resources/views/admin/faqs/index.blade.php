        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/faqs/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> {{__('admin.addfaq')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $faqs , 'selects' => ['id' , 'title' ], 'options' => ['show' , 'edit' , 'delete']])
                        @slot('paginate')
                            {{$faqs->links()}}
                        @endslot
                        @slot('url')
