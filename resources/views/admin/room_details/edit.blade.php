...
@section('main')
    <form class="form" method="post" id="mainForm" enctype="multipart/form-data"
          action="{{url('admin/room-details/update/'.$roomDetail->id)}}">
        @csrf
        <div class="row">
                    ...
                    @slot('items')
                        ...
                        @component('admin.components.form.optionLabel')
                            ...
                            @slot('items')
                                @foreach($projects as $project)
                                    <option value="{{$project->id}}" @if($project->id == $roomDetail->room->project->id) selected @endif>{{$project->name}}</option>
                                @endforeach
                            @endslot
                        @endcomponent
                        @component('admin.components.form.optionLabel')
                            ...
                            @slot('items')
                                @foreach($projects->first()->rooms as $room)
                                    <option value="{{$room->id}}" @if($project->id == $roomDetail->project_room_id) selected @endif>{{$room->name}}</option>
                                @endforeach
                            @endslot
                        @endcomponent
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                ...
                                <input type="file" name="new_picture" id="new_picture" class="dropify" data-max-file-size="500K"
                                       data-allowed-file-extensions="jpg png" data-default-file="{{url('/images/projects/'.$roomDetail->picture)}}">
                            </div>
                        </div>
                    @endslot
