        ...
        <div class="col-12" id="">
            @component('admin.components.panel')
                @slot('header')
                    ...
                    <a href="{{url('/admin/room-details/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i>
                            ...
                            <table class="table table-striped card-table table-vcenter text-nowrap table-bordered table-hover">
                                ...
                                @foreach($roomDetails as $roomDetail)
                                    <tr>
                                        ...
                                        <td>
                                            {{$roomDetail->room->project->name}}
                                        </td>
                                        <td>
                                            {{$roomDetail->room->name}}
                                        </td>
                                        <td>
                                            <a href="{{url('admin/room-details/delete/'. $roomDetail->id)}}"
                                               class="btn btn-sm btn-danger delete" data-toggle="tooltip"
                                               title data-placement="top" data-value="{{$roomDetail->id}}" data-original-title="Delete">
                                                <i class="fe fe-edit mr-2"></i>{{__('admin.delete')}}
                                            </a>
                                            <a href="{{url('admin/room-details/edit/' . $roomDetail->id)}}"
                                                   data-toggle="tooltip"
