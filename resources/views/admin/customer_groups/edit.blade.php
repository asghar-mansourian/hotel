...
@section('main')
    <form class="form" method="post" id="mainForm" enctype="multipart/form-data"
          action="{{url('admin/customer-groups/update/' . $customerGroup->id)}}">
        @csrf
        <div class="row">
                                ...
                                <div class="col-md-7">
                                    <select name="customer_ids[]" id="customer_ids[]" class="form-control select2 select2-hidden-accessible" multiple="" data-placeholder="Select Customers"  tabindex="-1" aria-hidden="true">
                                        @foreach($customers as $customer)
                                            <option value="{{$customer->id}}" @if(in_array($customer->id,$customerGroupCustomers)) selected @endif>{{$customer->name}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <input type="file" name="new_picture" id="picture" class="dropify" data-max-file-size="500K"
                                           data-allowed-file-extensions="jpg png" data-default-file="{{url('images/'.$customerGroup->image)}}">
                                </div>
                            </div>
                    @endslot
                ...
                @component('admin.components.panel')
                    ...
                    @slot('items')
                        @component('admin.components.form.submit')
                            ...
                            @endslot
                        @endcomponent
                            <a href="{{url('admin/customer-groups/')}}" class="btn btn-danger btn-block ">{{__('custom.other.cancel')}}</a>
                    @endslot
                @endcomponent
