            ...
            @component('admin.components.panel')
                @slot('header')
                    <h3 class="card-title" style="display: inline">
                        <i class="fa fa-clipboard   mr-2"></i>{{__('custom.admin.customer.groups.index.table.header')}}
                    </h3>
                    <a href="{{url('/admin/customer-groups/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i> Add
                        {{__('admin.customer_groups')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $customerGroups , 'selects' => ['id', 'name',], 'options' => ['edit' , 'delete']])
                        @slot('paginate')
