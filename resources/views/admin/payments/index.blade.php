...
@section('title')
    {{__('admin.dashboard')}}  | {{__('admin.paymenttitle')}}
@endsection
            ...
            @component('admin.components.panel')
                @slot('header')
                    <h3 class="card-title" style="display: inline">
                        <i class="fa fa-clipboard   mr-2"></i>{{__('admin.paymenttableheader')}}
                    </h3>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $payments , 'selects' => ['id' , 'price' , 'authority' ,'refid' ] , 'options' => ['show' , 'delete']])
                        @slot('paginate')
                            {{$payments->links()}}
                        @endslot
                        @slot('url')
...
@section('crumb')
    @component('admin.components.crumb')
        ...
        @slot('items')
            ...
            <li class="breadcrumb-item active"><i class="fe fe-paperclip mr-2 fs-14"></i>{{__('admin.paymenttitle')}}
            </li>
        @endslot
    @endcomponent
@endsection
