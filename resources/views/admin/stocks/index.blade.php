...
@section('title')
    {{__('admin.dashboard')}} | {{__('admin.stocktitle')}}
@endsection
            ...
            @component('admin.components.panel')
                @slot('header')
                    <h3 class="card-title" style="display: inline">
                        <i class="fa fa-clipboard   mr-2"></i>{{__('admin.stocktableheader')}}
                    </h3>
                    <a href="{{url('/admin/stocks/create')}}" class="btn btn-sm btn-info ml-auto"><i class="fe fe-plus-circle mr-1 "></i>{{__('admin.addstock')}} </a>
                @endslot

                @slot('items')
                    @component('admin.components.table' , ['sortType'=>$sortType,'sortField'=>$sortField,'records' => $stocks , 'selects' => ['id' , 'address' ], 'options' => [ 'edit' , 'delete']])
                        @slot('paginate')
                            {{$stocks->links()}}
                        @endslot
                        @slot('url')
...
@section('crumb')
    @component('admin.components.crumb')
        ...
        @slot('items')
            ...
            <li class="breadcrumb-item active"><i class="fe fe-paperclip mr-2 fs-14"></i>{{__('admin.stocktitle')}}
            </li>
        @endslot
    @endcomponent
@endsection
