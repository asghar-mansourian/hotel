@extends('members.layout')

@section('title')
    {{__('member.site_name')}}| {{__('member.payment_card')}}
@endsection

@section('main')
    @include('members.partials.top_panel')
@section('styles')
    <link rel="stylesheet" href="{{url('front/css/payment.css?v=1.0.0')}}">
    <style>
        @media screen and (max-width:680px){
            .card-body{
                padding-right: 80%!important;
            }
            .mt-5 {
                overflow-x: auto!important;
            }
        }
    </style>
@endsection
<div class="mt-5">
    <div class="border_sh courier_out" style="width: 872px;">
        <div class="row">
            <div class="hide-alert">
                @include('admin.components.error')
            </div>
        </div>
        <div class="card custom-card" style="border:1px solid var(--myColor);overflow-x:auto">
            <div class="card-body">
                <div class="checkout">
                    @include('members.partials.validation_errors')
                    <div class="credit-card-box">
                        <div class="flip">
                            <div class="front">
                                <div class="chip"></div>
                                <div class="logo1">
                                    <img
                                        src="data:image/svg+xml;utf8;base64,">
                                </div>
                                <div class="number"></div>
                                <div class="card-holder">
                                    <label>{{__('member.paymes_make_the_card')}}</label>
                                    <div></div>
                                </div>
                                <div class="card-expiration-date">
                                    <label>{{__('member.paymes_end_date')}}</label>
                                    <div></div>
                                </div>
                            </div>
                            <div class="back">
                                <div class="strip"></div>
                                <div class="logo">


                                </div>
                                <div class="ccv">
                                    <label>CCV</label>
                                    <div></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <form action="{{route('paymes.pay')}}" class="form" method="post" autocomplete="off">
                        @csrf
                        <input type="hidden" name="payment_id" value="{{$payment->id}}">
                        <p class="text-center"><strong>{{__('member.paymes_payment_amount')}} : <span style="color:var(--myColor);">{{$payment->price}} {{strtoupper($payment->balance_type)}}</span></strong></p>

                        <fieldset>
                            <label for="card-number">{{__('member.paymes_card_number')}}</label>
                            <input type="num" id="card-number" name="card-number" value="" class="input-cart-number" minlength="4" maxlength="4" required="">
                            <input type="num" id="card-number-1" name="card-number-1" value="" class="input-cart-number" minlength="4" maxlength="4" required="">
                            <input type="num" id="card-number-2" name="card-number-2" value="" class="input-cart-number" minlength="4" maxlength="4" required="">
                            <input type="num" id="card-number-3" name="card-number-3" value="" class="input-cart-number" minlength="4" maxlength="4" required="">
                        </fieldset>
                        <fieldset>
                            <label for="card-holder">{{__('member.paymes_make_the_card')}}</label>
                            <input type="text" id="card-holder" name="holder" value="" minlength="4" required="">
                        </fieldset>
                        <fieldset class="fieldset-expiration">
                            <label for="card-expiration-month">{{__('member.paymes_end_date')}}</label>
                            <div class="select">
                                <select id="card-expiration-month" name="expMonth" required="">
                                    <option></option>
                                    <option>01</option>
                                    <option>02</option>
                                    <option>03</option>
                                    <option>04</option>
                                    <option>05</option>
                                    <option>06</option>
                                    <option>07</option>
                                    <option>08</option>
                                    <option>09</option>
                                    <option>10</option>
                                    <option>11</option>
                                    <option>12</option>
                                </select>
                            </div>
                            <div class="select">
                                <select id="card-expiration-year" name="expYear" required="">
                                    <option></option>
                                    <option>2016</option>
                                    <option>2017</option>
                                    <option>2018</option>
                                    <option>2019</option>
                                    <option>2020</option>
                                    <option>2021</option>
                                    <option>2022</option>
                                    <option>2023</option>
                                    <option>2024</option>
                                    <option>2025</option>
                                </select>
                            </div>
                        </fieldset>
                        <fieldset class="fieldset-ccv">
                            <label for="card-ccv">CCV</label>
                            <input type="text" id="card-ccv" name="cvv" minlength="3" maxlength="3" value="" required="">
                        </fieldset>
                        <button class="btn" type="submit" name="submitCardInfo"><i class="fa fa-lock"></i> {{__('member.paymes_approved')}}
                        </button>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>

@endsection

@section('menuItem')
    @include('members.partials.menu_sidebar')
@endsection

@section('footerCustom')
    <script>
        $('.input-cart-number').on('keyup change', function () {
            $t = $(this);

            if ($t.val().length > 3) {
                $t.next().focus();
            }

            var card_number = '';
            $('.input-cart-number').each(function () {
                card_number += $(this).val() + ' ';
                if ($(this).val().length == 4) {
                    $(this).next().focus();
                }
            })

            $('.credit-card-box .number').html(card_number);
        });

        $('#card-holder').on('keyup change', function () {
            $t = $(this);
            $('.credit-card-box .card-holder div').html($t.val());
        });

        $('#card-holder').on('keyup change', function () {
            $t = $(this);
            $('.credit-card-box .card-holder div').html($t.val());
        });

        $('#card-expiration-month, #card-expiration-year').change(function () {
            m = $('#card-expiration-month option').index($('#card-expiration-month option:selected'));
            m = (m < 10) ? '0' + m : m;
            y = $('#card-expiration-year').val().substr(2, 2);
            $('.card-expiration-date div').html(m + '/' + y);
        })

        $('#card-ccv').on('focus', function () {
            $('.credit-card-box').addClass('hover');
        }).on('blur', function () {
            $('.credit-card-box').removeClass('hover');
        }).on('keyup change', function () {
            $('.ccv div').html($(this).val());
        });

        $('#card-number').change(function () {
            console.log(getCreditCardType($(this).val()));
        })


        setTimeout(function () {
            $('#card-number').focus().delay(1000).queue(function () {
                $(this).blur().dequeue();
            });
        }, 500);

        function getCreditCardType(accountNumber) {
            if (/^5[1-5]/.test(accountNumber)) {
                result = 'mastercard';
                // $('.logo').html('<img src="data:image/svg+xml;base64," height="55px">');
                $('.logo').html('<img src="data:image/svg+xml;utf8;base64," />');

            } else if (/^4/.test(accountNumber)) {
                result = 'visa';
                // $('.logo').html('<img src="data:image/svg+xml;base64," height="55px">');
                $('.logo').html('<img src="data:image/svg+xml;utf8;base64," />');

            } else if (/^(5018|5020|5038|6304|6759|676[1-3])/.test(accountNumber)) {
                result = 'maestro';
            } else {
                result = 'unknown'
                // $('.logo').html('<img src="data:image/svg+xml;base64," height="55px">');
                $('.logo').html('');

            }
            return result;
        }

    </script>
@endsection
